...
loadJSON(function (json) { paintingDatabase = JSON.parse(json); });
  ...
  parsedString.forEach(element => {
    if( element.startsWith("title:") ) {
      fp.push(paintingDatabase.filter(painting => {
        return (
          painting.title.toLowerCase().includes(element.replace("title:", ""))
        );
      }));
    } else if( element.startsWith("author:") ) {
      fp.push(paintingDatabase.filter(painting => {
        return (
          painting.author.toLowerCase().includes(element.replace("author:", ""))
        );
      }));
      ...
      fp.push(paintingDatabase.filter(painting => {
        return (
          painting.painting_type.toLowerCase().includes(element.replace("type:", ""))
        );
      }));
    } else {
      fp.push(paintingDatabase.filter(painting => {
